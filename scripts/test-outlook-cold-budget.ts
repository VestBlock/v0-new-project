...
assert.equal(OUTLOOK_COLD_B2B_LANE_KEYS.length, 7)
...
assert.equal(Object.values(allocation.laneCaps).filter((cap) => cap === 4).length, 4)
assert.equal(Object.values(allocation.laneCaps).filter((cap) => cap === 3).length, 3)
