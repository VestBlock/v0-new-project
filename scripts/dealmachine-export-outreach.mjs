...
function applyStrategyFilter(contacts) {
  ...
  if (TAX_CODE_STRATEGIES.has(STRATEGY)) {
    const hasTaxCodeExportSignal = (contact) =>
      /tax-code-stack|tax-delinquent-code-violation|code-tax-stack/i.test(String(contact.export_csv || "")) ||
      /tax-code-stack|tax-delinquent-code-violation|code-tax-stack/i.test(String(contact.source_file || ""))
    return annotated
      .filter((contact) =>
        TAX_CODE_STRATEGIES.has(normalizeMarketSlug(contact.queue_strategy_key)) ||
        hasTaxCodeExportSignal(contact) ||
        (
          String(contact.code_violation_hit || "").toLowerCase() === "true" &&
          Boolean(contact.tax_delinquent || contact.past_due_amount)
        )
