...
function getUrls() {
  ...
  const candidates = urlsArg
    ? urlsArg
        .split(',')
        .map((item) => item.trim())
        .filter(Boolean)
    : defaultPriorityPaths
...
async function inspectGoogleUrl(accessToken, siteUrl, inspectionUrl) {
...
async function main() {
  ...
  const urls = getUrls()
  const inspectLimit = Number.parseInt(getArgValue('--inspect-limit') || '5', 10)
  ...
  if (!googleConfig.configured) blockers.push('Google Search Console API credentials are missing.')
  if (!indexNowConfig.configured) {
    if (indexNowConfig.key && indexNowConfig.hostError) {
      blockers.push(`IndexNow host config is invalid (base URL parse failed): ${indexNowConfig.hostError}`)
    } else {
      blockers.push('IndexNow/Bing API key is missing.')
    }
  }

  if (!dryRun && googleConfig.configured) {
    try {
      const accessToken = await getGoogleAccessToken()
      await submitGoogleSitemap(accessToken, googleConfig.siteUrl, sitemapUrl)
      result.google.sitemapSubmitted = true
      for (const url of urls.slice(0, Math.max(0, inspectLimit))) {
        try {
  ...
  if (!dryRun && indexNowConfig.configured) {
    try {
      const status = await submitIndexNow(urls)
      result.indexNow.submitted = true
      result.indexNow.status = status
    } catch (error) {
      result.indexNow.error = error instanceof Error ? error.message : 'Unknown IndexNow push error.'
  ...
  const artifactDir = path.join(process.cwd(), 'artifacts', 'visibility-aeo')
  await mkdir(artifactDir, { recursive: true })
  const artifactPath = path.join(artifactDir, `visibility-indexing-push-${new Date().toISOString().slice(0, 10)}.json`)
