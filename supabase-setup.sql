...
-- Note: This part needs to be done in the Supabase dashboard or using the Supabase Management API
-- Create two buckets:
-- 1. credit-reports: For storing uploaded credit reports
-- 2. dispute-letters: For storing generated dispute letters

-- After creating the buckets in the dashboard, add these RLS policies for storage:

-- For credit-reports bucket
CREATE POLICY "Allow users to upload their own credit reports"
...
CREATE POLICY "Allow users to read their own credit reports"
...
CREATE POLICY "Allow users to upload their own dispute letters"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "Allow users to read their own dispute letters"
ON storage.objects FOR SELECT
TO authenticated
USING (
  bucket_id = 'dispute-letters' AND
