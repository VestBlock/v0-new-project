...
- 2026-07-02: SYNC DISCIPLINE IS NOW MANDATORY for every agent: start sessions with `git pull origin main` + this file + the changelog top; work on `codex/*` branches (Codex) or main-in-synced-folder (Claude); `npm run deploy:web:prod` now hard-gates on `scripts/sync-check.mjs` (main only, clean tree, not behind origin, unmerged agent branches surfaced). Rules: `docs/VESTBLOCK_SYNC_DISCIPLINE.md`, protocol: `.agents/skills/vestblock/sync-discipline-operator.md`. The retired cron-route daily loop in growth-automation-operator.md was replaced with the autopilot chain — do not recreate old cron routes.
- 2026-07-02: Reactivation batches send through `npm run outreach:send-reactivation` (preview) then `:live` — verified CSVs only, suppression/reply recheck at send time, CAN-SPAM footer, results feed the campaign ledger as source `reactivation`. A daily 7 AM scheduled task (`vestblock-outreach-morning-brief`, managed in the Cowork Scheduled sidebar) runs the autopilot and delivers the brief; it is instructed to never run live sends.
- 2026-07-02: Reply attribution is automated: `npm run outreach:reply-sync -- --apply` (also Phase 0 of the autopilot) matches inbox senders to ledger recipients, logs positives to the reply log + lead status, and auto-suppresses keyword opt-outs. Same gmail.readonly one-time grant as the export pipeline.
- 2026-07-02: DealMachine exports are automated: `npm run outreach:export-pipeline` builds today's brief lanes, requests the Contacts exports, watches Gmail for Export Complete, and downloads+ingests the CSVs into data/dm-exports. One-time prerequisite: GOOGLE_REFRESH_TOKEN must include gmail.readonly (run export-download --auth-url / --exchange-code --write-env once).
- 2026-07-02: Daily outreach now runs through `npm run outreach:autopilot` — one command that stages reactivation (Mondays), verifies emails, computes the learning audit, builds the send plan against OUTREACH_DAILY_CAP, and writes `data/operating-loops/morning-brief-latest.md` with blockers plus the single human decision. It never live-sends; the operator reviews and runs the printed send command.
- 2026-07-02: Inbound lead reliability pass. All four public intake routes now use Next 15 `after()` instead of fire-and-forget (Vercel was able to freeze the lambda and drop alerts/matching/automation), and the admin intake alert email is re-enabled opt-in (`sendIntakeAlert: true` on form routes only; import/scoring paths stay silent). Verify ADMIN_ALERT_EMAIL + RESEND_API_KEY exist in Vercel production env — they exist locally. SMS alert still requires Twilio + SELLER_LEAD_ALERT_PHONE, which are configured nowhere.
- 2026-07-02: Property analyzer now outputs `offerStrategy` (anchor/target/walk-away + approach + concession steps) and `repairSensitivity` (overrun scenarios, break-even repair budget); both render in the command-center analyzer panel and public analyzer. Log negotiation results with `npm run outreach:log-offer-outcome` so the Outcome Learning panel gets real data. Verify batches with `npm run outreach:verify-emails` before sends.
- 2026-07-02: Lead strategy memo added at `docs/VESTBLOCK_LEAD_STRATEGY_2026-07.md`. Key ledger finding: 88% of all sends went to the two now-paused lanes; high-intent lanes are undertested, so run the priority rotation at volume before adding new sources. New second-touch queue: `npm run outreach:reactivation-queue` (437 eligible, review-first, never sends). DealMachine public API remains a dead end for sourcing per the 2026-06-08 research; keep the website-export scripts.
- 2026-07-02: Reply attribution loop is live. Log every inbox reply with `npm run outreach:log-reply -- --email=... --status=replied|interested|qualified|closed_won|opt_out|wrong_owner|bounced`. Command-center strategy lanes now show real per-lane replies and opt-outs (suppression-file shape bug fixed), and `npm run outreach:learning-audit` is computed from the latest scorecard/ledger instead of hardcoded dates and narrative. See `docs/VESTBLOCK_REPLY_ATTRIBUTION_LOOP.md`.
