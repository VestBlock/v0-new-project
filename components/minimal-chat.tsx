...
export default function MinimalChat() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "system", content: "How can I help with your credit questions today?" },
  ])
  ...
  const sendMessage = async () => {
    ...
    try {
      // Get chat history in the format OpenAI expects
      ...
      if (data.success) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "system", content: `Error: ${data.error || "Failed to get response"}` },
        ])
      }
    } catch (error) {
      // Add error message
