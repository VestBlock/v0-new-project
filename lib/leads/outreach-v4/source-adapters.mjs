...
async function searchOutscraper({ vertical, market, niches, limitPerNiche, timeoutMs, websiteAuditLimit }) {
  ...
  const apiKey = normalizeText(process.env.OUTSCRAPER_API_KEY || process.env.DATAPIPE_API_KEY)
  if (!apiKey) return { provider: 'outscraper', ok: false, skipped: true, reason: 'missing_OUTSCRAPER_OR_DATAPIPE_API_KEY', leads: [] }
