...
export const REAL_SOURCE_DRY_RUN_QUARANTINE_REASON = 'paid_real_source_dry_run_quarantined'
const REAL_SOURCE_ADAPTERS_ENABLED = false

const OUTSCRAPER_API_BASE_URL = String(process.env.OUTSCRAPER_API_BASE_URL || 'https://api.datapipeplatform.cloud').replace(/\/+$/, '')

function isOutscraperEnabled() {
  return (
    /^(1|true|yes|on)$/i.test(String(process.env.ALLOW_PAID_SCRAPING || '').trim()) &&
...
async function searchOutscraper({ vertical, market, niches, limitPerNiche, timeoutMs, websiteAuditLimit }) {
  if (!isOutscraperEnabled()) {
    return { provider: 'outscraper', ok: false, skipped: true, reason: 'paid_outscraper_quarantined', leads: [] }
  }
  ...
  let response
  try {
    response = await fetch(`${OUTSCRAPER_API_BASE_URL}/google-maps-search`, {
      method: 'POST',
      headers: {
...
export async function runRealSourceAdapterV4(verticalPlan, options = {}) {
  if (!REAL_SOURCE_ADAPTERS_ENABLED) {
    return {
      ...
      ok: true,
      skipped: true,
      reason: REAL_SOURCE_DRY_RUN_QUARANTINE_REASON,
      rawFound: 0,
      leads: [],
      attempts: [],
    }
  ...
  for (const market of verticalPlan.markets) {
    ...
    const providers = providerPreference === 'auto'
      ? ['google_places', ...(isOutscraperEnabled() ? ['outscraper'] : [])]
      : [providerPreference]
