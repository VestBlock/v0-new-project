export const INVESTOR_HUNTER_BUDGET_TIME_ZONE = 'America/Chicago'
export const INVESTOR_HUNTER_BUDGET_HARD_LIMIT = 25

export type InvestorHunterBudgetMarker = {
...
export type InvestorHunterBudgetMetrics = {
...
export type NormalizedInvestorHunterBudget = {
  valid: boolean
  reason: string | null
  budgetDate: string
  attemptCount: number
  attemptMarkers: InvestorHunterBudgetMarker[]
}

export type InvestorHunterBudgetDecision = {
  allowed: boolean
  reason: string | null
  attemptCount: number
  remaining: number
...
export function investorHunterBudgetDateKey(now = new Date()) {
  const parts = new Intl.DateTimeFormat('en-US', {
    timeZone: INVESTOR_HUNTER_BUDGET_TIME_ZONE,
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  }).formatToParts(now)
...
function isBudgetMarker(value: Record<string, unknown>): value is InvestorHunterBudgetMarker {
  return (
    typeof value.reservationId === 'string' &&
    Boolean(value.reservationId) &&
    typeof value.claimId === 'string' &&
    Boolean(value.claimId) &&
    typeof value.investorId === 'string' &&
    Boolean(value.investorId) &&
    typeof value.reservedAt === 'string' &&
    Number.isFinite(Date.parse(value.reservedAt))
  )
}

export function normalizeInvestorHunterBudget(
  metrics: InvestorHunterBudgetMetrics | null | undefined,
  now = new Date()
): NormalizedInvestorHunterBudget {
  const currentDate = investorHunterBudgetDateKey(now)
  ...
  if (
    ...
    Number(count) < 0 ||
    Number(count) > INVESTOR_HUNTER_BUDGET_HARD_LIMIT ||
    markers.length !== rawMarkers.length ||
    markers.length !== Number(count)
  ) {
    return {
      valid: false,
  ...
  if (markers.some((marker) => investorHunterBudgetDateKey(new Date(marker.reservedAt)) !== storedDate)) {
    return {
      valid: false,
      reason: 'budget_marker_date_is_invalid',
      budgetDate: currentDate,
...
export function reserveInvestorHunterBudget(input: {
  metrics: InvestorHunterBudgetMetrics | null | undefined
  investorId: string
  claimId: string
  reservationId: string
  dailyLimit: number
  now?: Date
}): InvestorHunterBudgetDecision {
  const now = input.now || new Date()
  const dailyLimit = Number.isFinite(input.dailyLimit)
    ? Math.min(INVESTOR_HUNTER_BUDGET_HARD_LIMIT, Math.max(1, Math.floor(input.dailyLimit)))
    : 1
  const normalized = normalizeInvestorHunterBudget(input.metrics, now)
  const baseMetrics = {
  ...
  if (normalized.attemptMarkers.some((marker) => marker.claimId === input.claimId)) {
    return {
      allowed: false,
      reason: 'investor_hunter_claim_already_reserved',
      attemptCount: normalized.attemptCount,
...
export function nextInvestorHunterCasTimestamp(currentUpdatedAt: string, now = new Date()) {
