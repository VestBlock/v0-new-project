...
export const DEALMACHINE_STRATEGIES = [
  ...
  },
  {
    ...
    label: 'Absentee-owner portfolio candidate',
    markets: ['Memphis, TN', 'Kansas City, MO', 'Cleveland, OH', 'Toledo, OH', 'Indianapolis, IN', 'Louisville, KY'],
    reviewOnly: true,
    candidateOnly: true,
    candidateReason: 'DealMachine verifies absentee ownership; repeated ownership evidence is required before treating this lead as a portfolio owner.',
    enabled: true,
    lowball: false,
    budgetWeight: 0.08,
    variants: [variant('multiple-investments', ['absentee_owner'], [
  ...
  },
  {
    ...
    label: 'Small multifamily ownership candidate',
    markets: ['Kansas City, MO', 'Memphis, TN', 'Indianapolis, IN', 'Louisville, KY', 'Cleveland, OH', 'Toledo, OH'],
    reviewOnly: true,
    candidateOnly: true,
    candidateReason: 'DealMachine verifies a 2-20 unit property; repeated ownership evidence is required before treating the owner as a portfolio operator.',
    enabled: true,
    lowball: false,
    budgetWeight: 0.08,
    variants: [variant('two-to-twenty-units', ['multifamily'], [
...
function normalizedSourceType(value) {
  ...
  return ['properties', 'people'].includes(sourceType) ? sourceType : null
}

export function mergeDealMachineCatalogMetadata(propertyRows = [], peopleRows = []) {
  return [
    ...propertyRows.map((row) => ({ ...row, source_type: 'properties' })),
    ...peopleRows.map((row) => ({ ...row, source_type: 'people' })),
  ]
}
...
export function compileStrategyFilters(filterSpecs, filterMetadata, options = {}) {
  const preferredSourceType = normalizedSourceType(options.preferredSourceType) || 'properties'
  const metadataById = new Map()
  for (const row of filterMetadata) {
    const filterId = String(row.filter_id)
    const rows = metadataById.get(filterId) || []
    rows.push(row)
    metadataById.set(filterId, rows)
  }
  const warnings = []
  const filters = []
  const filterSources = []
  ...
  for (const spec of filterSpecs) {
    const candidates = metadataById.get(spec.filterId) || []
    ...
    const metadata = candidates.find((row) => normalizedSourceType(row.source_type) === preferredSourceType) || candidates[0]
    const compatibleSources = Array.from(new Set(
      candidates.map((row) => normalizedSourceType(row.source_type)).filter(Boolean)
    ))
    if (!compatibleSources.length) compatibleSources.push(preferredSourceType)
    ...
    filterSources.push({ filterId: spec.filterId, sourceTypes: compatibleSources })
  }

  const peopleOnlyFilterIds = filterSources
    .filter((entry) => entry.sourceTypes.includes('people') && !entry.sourceTypes.includes('properties'))
    .map((entry) => entry.filterId)
  const propertyFilterIds = filterSources
    .filter((entry) => entry.sourceTypes.includes('properties'))
    .map((entry) => entry.filterId)

  return { filters, warnings, filterSources, peopleOnlyFilterIds, propertyFilterIds }
}
...
export async function hydrateStrategyPlan(client, plan, filterMetadata) {
  ...
  const requestedAnchor = plan.anchor || 'properties'
  const [location, compiled] = await Promise.all([
    ...
    Promise.resolve(compileStrategyFilters(plan.variant.filters, filterMetadata, {
      preferredSourceType: requestedAnchor,
    })),
  ])
  const searchSourceType = requestedAnchor === 'people' || compiled.peopleOnlyFilterIds.length
    ? 'people'
    : 'properties'
  const anchor = searchSourceType === 'people' ? 'people' : requestedAnchor
  const commonBody = {
  ...
  const searchBody = searchSourceType === 'people'
    ? {
        ...commonBody,
        ...(compiled.propertyFilterIds.length ? { property_match: 'owner' } : {}),
      }
    : {
        ...commonBody,
        anchor,
        // Discovery and contact reveal are intentionally separate billable
        // phases. Property discovery must never spend people credits.
        contact_audience: 'none',
      }
  const exportBody = searchSourceType === 'people'
    ? {
        ...commonBody,
        ...(compiled.propertyFilterIds.length ? { property_match: 'owner' } : {}),
        anchor: 'person',
  ...
  return {
    ...
    filters: compiled.filters,
    filterSources: compiled.filterSources,
    peopleOnlyFilterIds: compiled.peopleOnlyFilterIds,
    propertyFilterIds: compiled.propertyFilterIds,
    warnings: compiled.peopleOnlyFilterIds.length
      ? [...compiled.warnings, `Routed people-only filters through DealMachine people search: ${compiled.peopleOnlyFilterIds.join(', ')}.`]
      : compiled.warnings,
    searchBody,
    exportBody,
  }
