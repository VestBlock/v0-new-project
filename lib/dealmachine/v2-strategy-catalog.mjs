import {
  DEALMACHINE_PROPERTY_FIELD_ALLOWLIST,
  DEALMACHINE_PROPERTY_FILTER_ALLOWLIST,
...
export const DEALMACHINE_STRATEGY_FIELDS = Object.freeze([
  ...DEALMACHINE_PROPERTY_FIELD_ALLOWLIST,
])

const APPROVED_PROPERTY_FILTERS = new Set(DEALMACHINE_PROPERTY_FILTER_ALLOWLIST)
...
export function compileStrategyFilters(filterSpecs, filterMetadata) {
  ...
  for (const spec of filterSpecs) {
    if (!APPROVED_PROPERTY_FILTERS.has(String(spec.filterId || ''))) {
      throw new Error(`DealMachine filter is not approved for property-only discovery: ${spec.filterId}`)
    }
...
export function buildDailyStrategyPlans(options = {}) {
  ...
  return DEALMACHINE_STRATEGIES
    ...
    .filter((strategy) => !strategy.lowball)
    .map((strategy) => ({
      ...strategy,
      date,
...
export async function hydrateStrategyPlan(client, plan, filterMetadata) {
  ...
  assertPropertyOnlySearchBody(searchBody, { requireFields: true })
  return {
    ...plan,
