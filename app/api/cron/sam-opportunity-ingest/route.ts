...
import { runSamOpportunityIngest } from '@/lib/sam/service'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await runSamOpportunityIngest({ dryRun })
    return NextResponse.json({ success: true, dryRun, ...result })
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'SAM opportunity ingest failed.' },
      { status: 500 }
    )
  }
}
