...
import { runSamExclusionRechecks } from '@/lib/sam/service'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await runSamExclusionRechecks({ dryRun })
    return NextResponse.json({ success: true, dryRun, ...result })
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'SAM exclusion rechecks failed.' },
      { status: 500 }
    )
  }
}
