...
import { runSamAssistanceRefresh } from '@/lib/sam/service'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await runSamAssistanceRefresh({ dryRun })
    return NextResponse.json({ success: true, dryRun, ...result })
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'SAM assistance refresh failed.' },
      { status: 500 }
    )
  }
}
