...
import { runSamMatchScoring } from '@/lib/sam/service'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await runSamMatchScoring({ dryRun })
    return NextResponse.json({ success: true, dryRun, ...result })
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'SAM match scoring failed.' },
      { status: 500 }
    )
  }
}
