...
import { runSamAwardMonitor } from '@/lib/sam/service'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await runSamAwardMonitor({ dryRun })
    return NextResponse.json({ success: true, dryRun, ...result })
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'SAM award monitor failed.' },
      { status: 500 }
    )
  }
}
