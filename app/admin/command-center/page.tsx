    ...
    <div className="mx-auto flex min-h-[60vh] w-full max-w-4xl flex-col justify-center px-6 py-16">
      <div className="rounded-3xl border border-amber-300/20 bg-[#11131a] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link
            href="/admin/leads"
            className="inline-flex items-center rounded-xl border border-cyan-300/25 bg-cyan-300/10 px-4 py-2 text-sm font-medium text-cyan-100 transition-colors hover:border-cyan-200/40 hover:bg-cyan-300/15"
          >
            Open leads
          </Link>
          <Link
...
export default async function CommandCenterPage() {
  ...
  let loaded: Awaited<ReturnType<typeof loadCommandCenterPage>> | null = null
  let errorMessage = ''
  try {
    loaded = await loadCommandCenterPage()
  } catch (error) {
    errorMessage = error instanceof Error ? error.message : 'Unable to load command center data.'
    console.error('[admin-command-center] render failed', error)
  }

  if (!loaded) return <AdminConsoleFallback message={errorMessage} />
  return <FounderCockpit {...loaded} />
}
...
async function loadCommandCenterPage() {
  ...
  return {
    initialData: data,
    initialCockpit: await getAutopilotCockpitSnapshot(data),
    initialBossBriefing: buildBossBriefing(data, learning),
  }
}
