  ...
  return (
    <div className="space-y-6 px-4 py-6 md:px-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm text-slate-400">Daily operating intelligence</p>
      ...
      {latest ? (
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <Card className="border-slate-800 bg-slate-950/70"><CardHeader><CardTitle className="text-sm text-slate-300">Automation</CardTitle></CardHeader><CardContent className="text-white">{String((latest.summary_json as any)?.automationStatus || 'n/a')}</CardContent></Card>
        </div>
      ) : null}
            ...
            <div key={report.id} className="flex flex-col gap-3 rounded border border-slate-800 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-2">
                  ...
                  <Badge variant="outline">{String((report.summary_json as any)?.automationStatus || 'Automation n/a')}</Badge>
                </div>
              </div>
